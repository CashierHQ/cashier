...
pub fn calculate_gate_fee_amount(gate_count: u64, max_use: u64, gate_create_fee: &Nat, gate_open_fee: &Nat) -> Nat {
    Nat::from(gate_count) * (gate_create_fee.clone() + Nat::from(max_use) * gate_open_fee.clone()).clone()
}
...
pub fn calculate_intent_total_amount(participants: IntentParticipants, user_input_amount: &Nat, max_use: u64, link_creation_fee: &Nat, link_max_asset_amount: &Nat, gate_count: u64, gate_create_fee: &Nat, gate_open_fee: &Nat) -> Nat {
    match participants {
