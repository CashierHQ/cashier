...
mod tests {
    ...
    #[tokio::test]
    async fn it_should_fail_create_action_if_action_type_is_not_withdraw_link() {
        // Arrange
        ...
        let result = state_handler
            .create_action(
                creator,
                ActionType::Receive, // Invalid action type for InactiveState
                transaction_manager,
                token_fee_service,
                token_standard_service,
                token_balance_service,
        ...
        match error {
            CanisterError::ValidationErrors(msg) => {
                assert_eq!(msg, "Unsupported action type for InactiveState".to_string())
            }
    ...
    #[tokio::test]
    async fn it_should_fail_process_action_if_action_type_is_not_withdraw_link() {
        // Arrange
        let creator = random_principal_id();
        let canister_id = random_principal_id();
        let ledger_id1 = random_principal_id();
        ...
        let action = Action {
            ...
            r#type: ActionType::Receive, // Invalid action type for InactiveState
            creator,
    ...
    async fn is_should_success_process_action_if_transaction_manager_succeeds() {
        ...
        assert_eq!(process_action_result.link.state, LinkState::InactiveEnded);
    }
}
