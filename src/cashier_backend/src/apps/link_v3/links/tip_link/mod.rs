...
mod tests {
    ...
    fn fixture_of_tip_link_with_state(
        state: LinkState,
        creator: Principal,
        canister_id: Principal,
        asset_info: Vec<AssetInfoV3>,
    ...
    #[tokio::test]
    async fn it_should_fail_create_action_due_to_unsupported_link_state_for_tip_link() {
        // Arrange
        let creator = random_principal_id();
        let canister_id = random_principal_id();
        let created_at = 1_000_000u64;
        let link = fixture_of_tip_link_with_state(
            LinkState::Ended,
            creator,
            canister_id,
    ...
    #[tokio::test]
    async fn it_should_fail_process_action_due_to_unsupported_link_state_for_tip_link() {
        // Arrange
        let creator = random_principal_id();
        let canister_id = random_principal_id();
        let created_at = 1_000_000u64;
    ...
    fn it_should_succeed_create_tip_link() {
        ...
        let canister_id = random_principal_id();
        let created_at = 1_000_000u64;
        let title = "Tip test".to_string();
        let asset_info = vec![fixture_of_asset_info_v3(
            random_principal_id(),
            Nat::from(1000u64),
        )];

        // Act
        let link = TipLink::create(
            creator,
            title.clone(),
            asset_info.clone(),
            created_at,
    ...
    #[tokio::test]
    async fn it_should_succeed_create_action_for_created_tip_link() {
        // Arrange
        let creator = random_principal_id();
        let canister_id = random_principal_id();
    ...
    #[tokio::test]
    async fn it_should_succeed_process_action_and_activate_created_tip_link() {
        // Arrange
        let creator = random_principal_id();
        let canister_id = random_principal_id();
        let ledger_id = random_principal_id();
