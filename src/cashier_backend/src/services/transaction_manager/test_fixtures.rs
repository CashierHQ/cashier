...
use cashier_backend_types::{dto::action::ActionDto, repository::{action::v1::{Action, ActionState, ActionType}, common::{Asset, Chain, Wallet}, 
intent::v2::{Intent, IntentState, IntentTask, IntentType, TransferData}, 
transaction::v2::{Transaction, TransactionState, Protocol, IcTransaction, Icrc1Transfer, FromCallType},
...
pub fn create_transaction_feature(service: &TransactionManagerService<MockIcEnvironment>) -> Transaction {
    ...
    service.action_service.transaction_repository.batch_create(vec![transaction.clone()]);
    transaction
}
...
pub fn create_action_data_fixture(
    ...
    intent_tx_map.insert(
        intent_id.clone(),
        vec![tx1.clone(), tx2.clone(), tx3.clone()],
    );

    service.action_service.store_action_data(link_action, action.clone(), intents.clone(), intent_tx_map, user_id).unwrap();
