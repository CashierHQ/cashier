use cashier_types::{StorableUserLinkKey, UserLink, UserLinkKey};
...
pub fn create(user_link: UserLink) {
    USER_LINK_STORE.with_borrow_mut(|store| {
        ...
        store.insert(id.into(), user_link);
    });
}
...
pub fn get(id: UserLinkKey) -> Option<UserLink> {
    USER_LINK_STORE.with_borrow(|store| store.get(&id.into()))
}
...
pub fn get_links_by_user_id(user_id: String, paginate: PaginateInput) -> PaginateResult<UserLink> {
    USER_LINK_STORE.with_borrow(|store| {
        let start: StorableUserLinkKey = (user_id.clone(), "".to_string()).into();
        let end: StorableUserLinkKey = (user_id, "\u{10FFFF}".to_string()).into(); // Use a high Unicode character to represent the end of the range
