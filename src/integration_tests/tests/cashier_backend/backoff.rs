...
#[tokio::test]
async fn it_should_throttle_on_two_separate_links_with_wrong_key() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange — two separate gated links; same receiver will hit both
        let admin = TestUser::CashierBackendAdmin.get_principal();
        let admin_client = ctx.new_cashier_backend_client(admin);
        ...
        let receiver_fixture =
            LinkTestFixtureV3::new(creator_fixture1.ctx.clone(), receiver, icp_fee).await;

        // First wrong attempt on link 1 — records failure #1, not yet throttled
        let _ = receiver_fixture
            .open_link_gate(&link_id1, &gate1.id, GateKey::Password("wrong".to_string()))
            .await;

        // Second wrong attempt on link 1 — now throttled
        let blocked_link1 = receiver_fixture
            .open_link_gate(&link_id1, &gate1.id, GateKey::Password("wrong".to_string()))
            .await;
        assert!(
            matches!(blocked_link1, Err(CanisterError::BackoffThrottled(_))),
            "expected BackoffThrottled on link 1 retry, got {blocked_link1:?}"
        );
        ...
        // Act — wrong attempt on link 2 (different link, same user)
        let blocked_link2 = receiver_fixture
            .open_link_gate(&link_id2, &gate2.id, GateKey::Password("wrong".to_string()))
            .await;

        // Assert — backoff is per-user, not per-link: link 2 is also throttled
        assert!(
            matches!(blocked_link2, Err(CanisterError::BackoffThrottled(_))),
            "expected BackoffThrottled on link 2 (backoff is per-user), got {blocked_link2:?}"
        );
...
#[tokio::test]
async fn it_should_bypass_backoff_when_base_wait_secs_is_zero() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange — base_wait_secs=0 makes next_allowed_ns == now_ns, so the
        // window check (now_ns < next_allowed_ns) is always false → no throttle
        let admin = TestUser::CashierBackendAdmin.get_principal();
        let admin_client = ctx.new_cashier_backend_client(admin);
        ...
        // First wrong attempt — records failure, consumes rate-limit slot
        let _ = receiver_fixture
            .open_link_gate(&link_id, &gate.id, GateKey::Password("wrong".to_string()))
            .await;
        ...
        // Act — immediate retry; backoff window is zero so it should not be throttled
        let result = receiver_fixture
            .open_link_gate(&link_id, &gate.id, GateKey::Password("wrong".to_string()))
            .await;

        // Assert — not BackoffThrottled (wrong-password or RateLimited, but never throttled)
        assert!(
            !matches!(result, Err(CanisterError::BackoffThrottled(_))),
            "expected no BackoffThrottled when base_wait_secs=0, got {result:?}"
        );
