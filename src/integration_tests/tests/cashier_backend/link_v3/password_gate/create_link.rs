...
use crate::{
    cashier_backend::link_v3::{
        fixture::LinkTestFixtureV3, password_gate::fixture::PasswordGateLinkFixture,
    },
    constant::ICP_TOKEN,
...
#[tokio::test]
async fn it_should_create_tip_link_without_gate() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
        let icp_ledger_client = ctx.new_icp_ledger_client(creator);
        ...
        let result = link_fixture
            ...
            .await
            .expect("should succeed with no gates");

        // Assert
        assert!(!result.link.id.is_empty());
        assert_eq!(result.gates.len(), 0, "no gates expected for ungated link");
