...
#[tokio::test]
async fn it_should_fail_disable_icp_token_airdrop_link_v3_if_link_not_active() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        ...
        let (test_fixture, create_link_result) = create_airdrop_link_v3_fixture(
            ctx,
            caller,
            token,
            airdrop_amount,
...
#[tokio::test]
async fn it_should_fail_disable_icp_token_airdrop_link_v3_if_caller_is_not_creator() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let token = ICP_TOKEN;
...
#[tokio::test]
async fn it_should_succeed_disable_icp_token_airdrop_link_v3() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let airdrop_amount = Nat::from(1_000_000u64);
...
#[tokio::test]
async fn it_should_succeed_disable_icrc_token_airdrop_link_v3() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
