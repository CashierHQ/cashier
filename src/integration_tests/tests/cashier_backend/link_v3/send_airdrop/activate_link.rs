...
use crate::{
    cashier_backend::link_v3::{
        ...
        send_airdrop::fixture::{AirdropLinkV3Fixture, activate_airdrop_link_v3_fixture},
    },
...
#[tokio::test]
async fn it_should_fail_activate_icp_token_airdrop_link_v3_if_caller_anonymous() {
...
#[tokio::test]
async fn it_should_fail_activate_icp_token_airdrop_link_v3_if_caller_not_creator() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
...
#[tokio::test]
async fn it_should_fail_activate_icp_token_airdrop_link_v3_if_link_not_exists() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let token = ICP_TOKEN;
...
#[tokio::test]
async fn it_should_succeed_activate_icp_token_airdrop_link_v3() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let token = ICP_TOKEN;
...
#[tokio::test]
async fn it_should_succeed_activate_icrc_token_airdrop_link_v3() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
...
#[tokio::test]
async fn it_should_succeed_activate_icrc_token_airdrop_link_v3_fixture() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let airdrop_amount = Nat::from(5_000_000u64);
        ...
        // Act
        let (_fixture, result) = activate_airdrop_link_v3_fixture(
            ctx,
            CKBTC_ICRC_TOKEN,
            airdrop_amount,
            max_use_count,
