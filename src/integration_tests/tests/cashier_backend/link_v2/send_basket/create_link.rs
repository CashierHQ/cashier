...
async fn it_should_error_create_icp_token_basket_linkv2_if_caller_anonymous() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        ...
        let test_fixture = BasketLinkV2Fixture::new(
            Arc::new(ctx.clone()),
            caller,
            tokens.clone(),
            amounts.clone(),
        ...
        let input = test_fixture
            .token_basket_link_input(tokens, amounts)
            .unwrap();
...
async fn it_should_error_when_create_createlink_action_twice() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange: create basket link but do not activate
        let caller = TestUser::User1.get_principal();
        let tokens = vec![constant::ICP_TOKEN.to_string()];
        let amounts = vec![Nat::from(1_000_000u64)];
        let test_fixture =
...
#[tokio::test]
async fn it_should_create_icp_token_basket_linkv2_successfully() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let tokens = vec![constant::ICP_TOKEN.to_string()];
        ...
        // intent_total_amount = transfer amount = (amount + fee) * max_use = amount + fee (max_use=1)
        let expected_total_amount_intent1 = amounts[0].clone() + icp_ledger_fee.clone();
        assert_eq!(
            intent1.intent_total_amount,
            Some(expected_total_amount_intent1),
            "Intent 1 total amount should equal transfer amount"
        );
        // intent_total_network_fee = fee * 2 (ICRC1: inbound + outbound)
        let expected_network_fee_intent1 = icp_ledger_fee.clone() * Nat::from(2u64);
        assert_eq!(
            intent1.intent_total_network_fee,
            Some(expected_network_fee_intent1.clone()),
            "Intent 1 network fee should be fee * 2 for ICRC1"
        );
...
#[tokio::test]
async fn it_should_create_icrc_token_basket_linkv2_successfully() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        ...
        // Fee assertions - Intent 1 (TransferWalletToLink)
        // intent_total_amount = transfer amount = (amount + fee) * max_use = amount + fee (max_use=1)
        let expected_total_amount_intent1 = amounts[0].clone() + ckbtc_ledger_fee.clone();
        assert_eq!(
            intent1.intent_total_amount,
            Some(expected_total_amount_intent1),
            "Intent 1 total amount should equal transfer amount"
        ...
        // intent_total_network_fee = fee * 2 (ICRC1: inbound + outbound)
        let expected_network_fee_intent1 = ckbtc_ledger_fee.clone() * Nat::from(2u64);
        assert_eq!(
            intent1.intent_total_network_fee,
            Some(expected_network_fee_intent1.clone()),
            "Intent 1 network fee should be fee * 2 for ICRC1"
