...
#[tokio::test]
async fn it_should_succeed_get_icp_token_basket_linkv2_with_no_link_existed() {
...
#[tokio::test]
async fn it_should_succeed_get_icp_token_basket_linkv2_with_no_paginate_option() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
...
#[tokio::test]
async fn it_should_succeed_get_icp_token_basket_linkv2_with_paginate_option() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let tokens = vec![ICP_TOKEN.to_string()];
        let amounts = vec![Nat::from(1_000_000u64)];
