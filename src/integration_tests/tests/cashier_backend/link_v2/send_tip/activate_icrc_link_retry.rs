...
async fn it_should_create_new_icrc112_with_proper_created_time_if_activate_twice_within_1_day() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        ...
        let icp_icrc2_approve_request = requests
            .iter()
            .find(|req| req.method == "icrc2_approve" && req.canister_id == Principal::from_text(ICP_PRINCIPAL).unwrap())
            .expect("icrc2_approve request not found");
        let first_icp_icrc2_approve_arg = Decode!(&icp_icrc2_approve_request.arg, ApproveArgs)
            .expect("Failed to decode icrc2_approve args");

        let ckbtc_icrc2_approve_request = requests
            .iter()
            .find(|req| req.method == "icrc2_approve" && req.canister_id == Principal::from_text(CK_BTC_PRINCIPAL).unwrap())
            .expect("ckbtc icrc2_approve request not found");
        let first_ckbtc_icrc2_approve_arg = Decode!(&ckbtc_icrc2_approve_request.arg, ApproveArgs)
        ...
        let icp_icrc2_approve_request = requests
            ...
            .find(|req| req.method == "icrc2_approve" && req.canister_id == Principal::from_text(ICP_PRINCIPAL).unwrap())
            .expect("icrc2_approve request not found");
        let second_icp_icrc2_approve_arg = Decode!(&icp_icrc2_approve_request.arg, ApproveArgs)
            .expect("Failed to decode icrc2_approve args");
        ...
        let ckbtc_icrc2_approve_request = requests
            ...
            .find(|req| req.method == "icrc2_approve" && req.canister_id == Principal::from_text(CK_BTC_PRINCIPAL).unwrap())
            .expect("ckbtc icrc2_approve request not found");
        let second_ckbtc_approve_arg = Decode!(&ckbtc_icrc2_approve_request.arg, ApproveArgs)
            .expect("Failed to decode ckbtc icrc2_approve args");

        assert_eq!(
            second_icp_icrc2_approve_arg, first_icp_icrc2_approve_arg,
            "ICP ICRC2 approve args should be the same on second activation"
        );
        assert_eq!(
            second_ckbtc_approve_arg, first_ckbtc_icrc2_approve_arg,
            "CKBTC ICRC2 approve args should be the same on second activation"
        );
        ...
        let icp_icrc2_approve_request = requests
            ...
            .find(|req| req.method == "icrc2_approve" && req.canister_id == Principal::from_text(ICP_PRINCIPAL).unwrap())
            .expect("icrc2_approve request not found");
        let third_icp_icrc2_approve_arg = Decode!(&icp_icrc2_approve_request.arg, ApproveArgs)
            .expect("Failed to decode icrc2_approve args");
        ...
        let ckbtc_icrc2_approve_request = requests
            ...
            .find(|req| req.method == "icrc2_approve" && req.canister_id == Principal::from_text(CK_BTC_PRINCIPAL).unwrap())
            .expect("ckbtc icrc2_approve request not found");
        let third_ckbtc_icrc2_approve_arg = Decode!(&ckbtc_icrc2_approve_request.arg, ApproveArgs)
            .expect("Failed to decode ckbtc icrc2_approve args");

        let third_icp_approve_created_ts = third_icp_icrc2_approve_arg
            .created_at_time
            .expect("Forth icrc2_approve args ts should not be None");
        let second_icp_approve_created_ts = second_icp_icrc2_approve_arg
            .created_at_time
            .expect("Third icrc2_approve args ts should not be None");
        assert!(
            (third_icp_approve_created_ts - second_icp_approve_created_ts) > (24 * 3600 + 1) * 1_000_000_000 ,
            "ICP ICRC2 approve created_at_time should be advanced by 1 day + 1 second on third activation"
        );

        let third_ckbtc_approve_created_ts = third_ckbtc_icrc2_approve_arg
            .created_at_time
            .expect("Forth ckbtc icrc2_approve args ts should not be None");
        let second_ckbtc_approve_created_ts = second_ckbtc_approve_arg
            .created_at_time
            .expect("Third ckbtc icrc2_approve args ts should not be None");
        assert!(
            (third_ckbtc_approve_created_ts - second_ckbtc_approve_created_ts) > (24 * 3600 + 1) * 1_000_000_000 ,
            "CKBTC ICRC2 approve created_at_time should be advanced by 1 day + 1 second on third activation"
        );
