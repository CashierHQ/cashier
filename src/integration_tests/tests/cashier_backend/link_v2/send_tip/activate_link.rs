...
use crate::constant::{CKBTC_ICRC_TOKEN, ICP_TOKEN, TESTICP_ICRC_TOKEN};
...
#[tokio::test]
async fn it_should_succeed_activate_icrc2_token_tip_link() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
...
#[tokio::test]
async fn it_should_succeed_activate_icrc1_token_tip_link() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
        let token = TESTICP_ICRC_TOKEN;
        let tip_amount = Nat::from(5_000_000u64);
        let mut test_fixture =
            TipLinkV2Fixture::new(Arc::new(ctx.clone()), creator, token, tip_amount.clone()).await;
        let create_link_result = test_fixture.create_link().await;
        ...
        let icp_ledger_client = ctx.new_icp_ledger_client(creator);
        let icrc1_ledger_client = ctx.new_icrc_ledger_client(TESTICP_ICRC_TOKEN, creator);
        ...
        // Assert: Link balance after activation
        let link_account = link_id_to_account(ctx, &link_id);
        let icrc1_link_balance = icrc1_ledger_client.balance_of(&link_account).await.unwrap();
        let icrc1_ledger_fee = icrc1_ledger_client.fee().await.unwrap();
