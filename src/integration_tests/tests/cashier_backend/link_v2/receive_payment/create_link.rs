...
#[tokio::test]
async fn it_should_error_create_icp_token_payment_linkv2_if_caller_anonymous() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        ...
        let input = test_fixture.payment_link_input().unwrap();
...
#[tokio::test]
async fn it_should_create_icp_token_payment_linkv2_successfully() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
        let tokens = vec![constant::ICP_TOKEN.to_string()];
        ...
        let mut test_fixture =
            PaymentLinkV2Fixture::new(Arc::new(ctx.clone()), caller, tokens, amounts.clone()).await;
        ...
        assert_eq!(
            ...
            amounts[0].clone(),
            "Receive payment amount does not match"
        );

        assert_eq!(action.intents.len(), 1);

        // Assert Intent 1: TransferWalletToTreasury
        ...
        // Fee assertions - Intent 1 (TransferWalletToTreasury)
        // intent_total_amount = CREATE_LINK_FEE
        assert_eq!(
            intent1.intent_total_amount,
            Some(Nat::from(CREATE_LINK_FEE)),
            "Intent total amount should equal CREATE_LINK_FEE"
        );
        // intent_total_network_fee = fee * 2 (approve + transfer_from)
        let expected_network_fee = icp_ledger_fee.clone() * Nat::from(2u64);
        assert_eq!(
            intent1.intent_total_network_fee,
            Some(expected_network_fee.clone()),
            "Intent network fee should be fee * 2 for ICRC2 flow"
        );
        // intent_user_fee = total_amount + network_fee for creator->treasury
        let expected_user_fee = Nat::from(CREATE_LINK_FEE) + expected_network_fee;
        assert_eq!(
            intent1.intent_user_fee,
            Some(expected_user_fee),
            "Intent user fee should equal total_amount + network_fee"
        );
...
#[tokio::test]
async fn it_should_create_icrc_token_payment_linkv2_successfully() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let caller = TestUser::User1.get_principal();
