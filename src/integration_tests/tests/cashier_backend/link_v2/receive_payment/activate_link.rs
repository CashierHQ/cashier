...
#[tokio::test]
async fn it_should_fail_activate_icp_token_payment_linkv2_if_caller_anonymous() {
...
#[tokio::test]
async fn it_should_fail_activate_icp_token_payment_linkv2_if_caller_not_creator() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
        let tokens = vec![ICP_TOKEN.to_string()];
...
#[tokio::test]
async fn it_should_fail_activate_icp_token_payment_linkv2_if_link_not_exists() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
        let tokens = vec![ICP_TOKEN.to_string()];
...
#[tokio::test]
async fn it_should_succeed_activate_icp_token_payment_linkv2() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
        let tokens = vec![ICP_TOKEN.to_string()];
        ...
        let mut test_fixture =
            PaymentLinkV2Fixture::new(Arc::new(ctx.clone()), creator, tokens, amounts.clone())
                .await;
        let create_link_result = test_fixture.create_link().await;
...
#[tokio::test]
async fn it_should_succeed_activate_icrc_token_payment_linkv2() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = TestUser::User1.get_principal();
