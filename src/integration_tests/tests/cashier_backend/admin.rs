...
async fn should_allow_admin_to_set_and_remove_permissions() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        ...
        // Act
            ...
            let user_permissions_get_1 = admin_client
                .admin_permissions_get(user)
                .await
                .unwrap();
            ...
            let user_permissions_get_2 = admin_client
                .admin_permissions_get(user)
                .await
                .unwrap();
...
async fn should_not_allow_to_set_and_remove_permissions() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        ...
        // Act
            ...
            // Assert
            assert!(user_permissions_add.unwrap_err().to_string().contains("NotAuthorized"));
            assert!(user_permissions_remove.unwrap_err().to_string().contains("NotAuthorized"));
