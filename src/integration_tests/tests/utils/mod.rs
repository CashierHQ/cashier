use crate::{
    ckbtc,
    constant::{
        self, CK_BTC_PRINCIPAL, CK_ETH_PRINCIPAL, CK_USDC_PRINCIPAL, ICP_PRINCIPAL,
        ICRC7_NFT_PRINCIPAL, TESTICP_PRINCIPAL,
    },
...
async fn deploy_template_state(template_dir: &Path) -> SharedPrincipals {
    ...
    // Deploy gate_service first so cashier_backend can be initialized with its real principal.
