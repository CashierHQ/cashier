use crate::{
    gate_service::fixtures::add_xretweetedpost_gate_fixture,
...
#[tokio::test]
async fn it_should_fail_open_xretweetedpost_gate_due_to_wrong_key_type() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = random_principal_id();
        let subject_id = random_id_string();
        let gate = add_xretweetedpost_gate_fixture(ctx, creator, &subject_id, TWEET_URL).await;
        let user = TestUser::User1.get_principal();
        let user_client = ctx.new_gate_service_client(user);

        // Act - provide wrong key type (Password instead of XRetweetedPostCredential)
...
#[tokio::test]
async fn it_should_fail_open_xretweetedpost_gate_due_to_tweet_not_retweeted() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = random_principal_id();
        let subject_id = random_id_string();
        let gate =
            add_xretweetedpost_gate_fixture(ctx, creator, &subject_id, TWEET_URL).await;
        let user = TestUser::User1.get_principal();
        ...
        // Mock the X API response: user's tweets do NOT include a retweet of our tweet
        ...
        let body = r#"{"data":[{"id":"rt_111","referenced_tweets":[{"type":"retweeted","id":"999999999"}]}]}"#;
        ctx.client
        ...
        // Assert - should fail because none of the user's tweets retweet the target tweet
        assert!(result.is_err());
...
#[tokio::test]
async fn it_should_fail_open_xretweetedpost_gate_due_to_api_error() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = random_principal_id();
        let subject_id = random_id_string();
...
#[tokio::test]
async fn it_should_add_xretweetedpost_gate() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = random_principal_id();
        let subject_id = random_id_string();
        ...
        assert_eq!(gate.key, GateKey::XRetweetedPost(TWEET_URL.to_string()));
...
#[tokio::test]
async fn it_should_open_xretweetedpost_gate() {
    with_pocket_ic_context::<_, ()>(async move |ctx| {
        // Arrange
        let creator = random_principal_id();
        let subject_id = random_id_string();
        ...
        // Mock the X API response: user's tweets include a retweet of our tweet
        let pending_http = ctx.client.get_canister_http().await;
        assert!(!pending_http.is_empty(), "Expected a pending HTTP request");
        let request = &pending_http[0];
        let body = format!(
            r#"{{"data":[{{"id":"rt_111","referenced_tweets":[{{"type":"retweeted","id":"{}"}}]}}]}}"#,
            TWEET_ID
        );
        ctx.client
            .mock_canister_http_response(MockCanisterHttpResponse {
