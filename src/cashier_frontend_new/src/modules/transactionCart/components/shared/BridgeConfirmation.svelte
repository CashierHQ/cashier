      ...
      {#each Array(minConfirmations) as _, index (index)}
        ...
        <div class="flex justify-between items-start">
          <div class="flex items-center gap-3">
            ...
            {/if}
            <p class="text-base font-medium text-gray-700">Confirmation #{index + 1}</p>
          </div>
          
          <div class="flex flex-col items-end">
            {#if confirmation}
              <p class="text-base font-medium">
                {new Date(Number(confirmation.block_timestamp) * 1000).toLocaleDateString('en-US', { 
                  month: 'short', 
                  day: 'numeric', 
                  year: 'numeric' 
                })}
              ...
              <p class="text-sm text-gray-400">
                {new Date(Number(confirmation.block_timestamp) * 1000).toLocaleTimeString('en-US', { 
                  hour: '2-digit', 
                  minute: '2-digit',
                  hour12: false 
                })}
