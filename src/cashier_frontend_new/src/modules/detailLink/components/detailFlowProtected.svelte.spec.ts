...
describe('DetailFlowProtected', () => {
  ...
  it('renders children when step is allowed and user is creator', async () => {
    ...
    expect((goto as unknown as { mock: { calls: unknown[] } }).mock.calls.length).toBe(0);
  });

  it('redirects when step is not allowed', async () => {
    const linkStore = createMockLinkDetailStore(LinkStep.CHOOSE_TYPE);

    const snip = createRawSnippet(() => ({ render: () => `<div data-testid="slot">child</div>` }));
  ...
  it('redirects when user is not the creator', async () => {
    const linkStore = createMockLinkDetailStore(LinkStep.ACTIVE, 'different-owner-principal');
  ...
  it('renders children for all allowed steps', async () => {
    const allowedSteps = [LinkStep.CREATED, LinkStep.ACTIVE, LinkStep.INACTIVE, LinkStep.ENDED];
