...
function makeSampleLink(id = "link-1", title = "Test Tip Link") {
  ...
  const assetInfo = new AssetInfo(Asset.IC(assetAddress), BigInt(5000000), "SEND_TIP_ASSET");
...
describe("LinkMapper serde", () => {
  it("serializes and deserializes Link via LinkMapper.serde", () => {
    ...
    expect(deserialized.asset_info[0].amount_per_link_use_action).toBe(link.asset_info[0].amount_per_link_use_action);
    expect(deserialized.asset_info[0].asset.chain).toBe(link.asset_info[0].asset.chain);
    expect(deserialized.asset_info[0].asset.address?.toText()).toBe(link.asset_info[0].asset.address?.toText());
  ...
  it("stringify and parse a Link using devalue and LinkMapper.serde", () => {
    ...
    const serialized = devalue.stringify(link as unknown, LinkMapper.serde.serialize);
    expect(typeof serialized).toBe("string");

    const parsed = devalue.parse(serialized, LinkMapper.serde.deserialize) as Link;
    ...
    expect(parsed.asset_info[0].amount_per_link_use_action).toBe(link.asset_info[0].amount_per_link_use_action);
    expect(parsed.asset_info[0].asset.chain).toBe(link.asset_info[0].asset.chain);
    expect(parsed.asset_info[0].asset.address?.toText()).toBe(link.asset_info[0].asset.address?.toText());
