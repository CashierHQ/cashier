...
class TokenMetadataService {
    ...
    public async getTokenMetadata(tokenAddres: string): Promise<IcrcTokenMetadata | undefined> {
    ...
    public async getTokenMetadataBatch(tokenAddresses: [string]): Promise<Record<string, (IcrcTokenMetadata | undefined)>> {
        const agent = new HttpAgent({
            host: HOST_ICP,
        });
        ...
        const requests = tokenAddresses.map((tokenAddress) => {
            ...
            return ledger.metadata({}).then((response) => [tokenAddress, mapTokenMetadata(response)] as const);
        });
