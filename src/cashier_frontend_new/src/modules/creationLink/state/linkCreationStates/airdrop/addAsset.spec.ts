...
import { AddAssetAirdropState } from "./addAsset";
  ...
  locale: {
    t: vi.fn((key: string) => {
      const translations: Record<string, string> = {
        "links.linkForm.addAsset.errors.assetRequired": "Asset is required",
        "links.linkForm.addAsset.errors.onlyOneAssetSupported":
          "Only one asset is supported",
...
describe("AddAssetAirdropState", () => {
  ...
  describe("goNext()", () => {
    it("should transition to PREVIEW successfully with valid asset", async () => {
      ...
      // Arrange: provide valid airdrop link details
    ...
    it("should throw error when assets array is empty", async () => {
      ...
      // Assert
      await expect(store.goNext()).rejects.toThrow("Asset is required");
    });
  ...
  describe("state properties", () => {
    it("should have step equal to ADD_ASSET", () => {
      // Arrange
      const tempLink = new TempLink(
        "test-id",
        BigInt(Date.now()),
