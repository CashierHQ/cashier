...
export class AddAssetState implements LinkCreationState {
  ...
  async goNext(): Promise<void> {
    ...
    const isTokenBasket =
      this.#link.createLinkData.linkType === LinkType.TOKEN_BASKET;

    // For non-TokenBasket types, only one asset is supported
    if (!isTokenBasket && this.#link.createLinkData.assets.length > 1) {
      throw new Error(
