...
export class AddAssetTokenBasketSharedTestState implements LinkCreationState {
  ...
  constructor(link: LinkCreationStore) {
    ...
    if (
      ...
      if (!ok) {
        throw new Error(
          "Failed to initialize action from template for AddAssetTokenBasketSharedTestState",
        );
      }
    }
  }
  ...
  async goNext(): Promise<void> {
    ...
    const params = this.#link.createLinkData.assets.map((asset) => {
      const assetAddressStr = asset.address;
      const useAmount = asset.useAmount;
      const assetToken = tokens.find((t) => t.address === assetAddressStr);
      const assetNetworkFee = assetToken?.fee ?? ICP_LEDGER_FEE;
      const tokenStandard = TokenStandard.ICRC2; // TODO: need to determine token standard by looking up in the registry
    ...
    console.log("Updating asset intent with params:", params);

    this.#link.updateAssetIntent(params);
