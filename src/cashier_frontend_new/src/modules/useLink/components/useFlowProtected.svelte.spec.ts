...
describe("UseFlowProtected", () => {
  ...
  it("renders children when on /use page, logged in, and link is active", async () => {
    ...
    // Wait for the redirect check effect to complete
    await waitFor(() => {
  ...
  it("shows loading when link is null before redirect", async () => {
  ...
  describe("redirect logic", () => {
    it("redirects to landing when on /use page and not logged in", async () => {
      ...
      const gotoMock = (goto as unknown as { mock: { calls: unknown[][] } })
        .mock;
    ...
    it("redirects to /links when link is null after loading completes", async () => {
      mockUserProfile.isLoggedIn.mockReturnValue(true);
      mockPage.url.pathname = "/link/test-link-id";
    ...
    it("redirects to landing when on /use page, logged in, link is inactive, and user is not completed", async () => {
      mockUserProfile.isLoggedIn.mockReturnValue(true);
      mockPage.url.pathname = "/link/test-link-id/use";
    ...
    it("redirects to landing when on /use page, logged in, link is ended, and user is not completed", async () => {
      mockUserProfile.isLoggedIn.mockReturnValue(true);
      mockPage.url.pathname = "/link/test-link-id/use";
