...
describe.only("Result", () => {
  ...
  it("should convert a Response.ok to a Result", () => {
      ...
      // Assert
      expect(result.unwrapErr()).toEqual(new Error(devalue.stringify("error_message")));
    });
  ...
  it("should convert a Response.Err to a Result", () => {
      ...
      // Assert
      expect(result.unwrapErr()).toEqual(new Error(devalue.stringify("ErrorMessage")));
  });

  it("should catch a valid response", () => {
      let result = catchError(() => {
