<script lang="ts">
  ...
  onMount(async () => {
    ...
    derivationOrigin =
      new URLSearchParams(window.location.search).get("derivationOrigin") ??
      undefined;
...
{#if isLoginPopup}
  ...
  <main
    style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: #f5f7fa;"
  >
    <div style="text-align: center; padding: 2rem;">
      {#if loginPending}
        ...
        <p style="font-size: 0.85rem; color: #999; margin-top: 0.5rem;">
          Please complete authentication in the Internet Identity window.
        </p>
      {:else if authenticated}
        <p style="font-size: 1.1rem; color: green;">
...
{:else}
  ...
  <main style="font-family: monospace; padding: 1rem; max-width: 600px;">
    <h1>
      Wallet <small style="font-size: 0.6em; color: #888;">port 5177</small>
    </h1>

    <!-- Auth section -->
    <section
      style="margin-bottom: 1.5rem; padding: 1rem; border: 1px solid #ccc; border-radius: 6px;"
    >
      <h2>Identity</h2>
      {#if authenticated}
        <p style="color: green;">Authenticated</p>
        <p style="word-break: break-all; font-size: 0.85em;">
          Principal: {principal}
        </p>
    ...
    <section style="padding: 1rem; border: 1px solid #ccc; border-radius: 6px;">
      ...
      {#if requestLog.length === 0}
        <p style="color: #888; font-size: 0.85em;">
          Waiting for postMessage requests...
        </p>
      {:else}
