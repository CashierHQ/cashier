  ...
  switch (req.method) {
    ...
    case "icrc1_balance_of": {
      ...
      if (!bp.canisterId) {
        respond(undefined, {
          code: -32602,
          message: "Missing params.canisterId",
        });
        return;
      }
      if (!bp.consentId) {
        respond(undefined, {
          code: 4001,
          message: "icrc1_balance_of requires consent: missing consentId",
        });
        return;
      }
      ...
      try {
        ...
        const resolvedOwner =
          bp.owner ?? getIdentity()?.getPrincipal().toText();
        if (!resolvedOwner) {
          respond(undefined, {
            code: -32603,
            message: "No identity available",
          });
          return;
        }
    ...
    case "icrc1_transfer": {
      ...
      if (!tp?.canisterId || !tp?.to || !tp?.amount) {
        respond(undefined, {
          code: -32602,
          message: "Missing params.canisterId, params.to, or params.amount",
        });
        return;
      }
      if (!tp.consentId) {
        respond(undefined, {
          code: 4001,
          message: "icrc1_transfer requires consent: missing consentId",
        });
        return;
      }
