...
describe("Test create and claim token basket link", () => {
    ...
    describe("With Alice", () => {
        ...
        it("should complete the full link creation process", async () => {
            ...
            for (const asset of assets) {
                ...
                const transferAmount = (asset.amount_per_link_use! + ledger_fee) * max_use;
                const additionalFee =
                    asset.tokenIndex === "ICP" ? approveAmount + ledger_fee : BigInt(0);
                const expectedBalanceAfter = balanceBefore - transferAmount - additionalFee;
    ...
    describe("With Bob", () => {
        ...
        it("should create use action", async () => {
            ...
            expect(intent.type).toEqual("MultiTransfer");
            expect(intent.chain).toEqual("IC");
            expect(intent.task).toEqual("multi_transfer_link_to_wallet");
