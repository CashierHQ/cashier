...
export const idlFactory = ({ IDL }) => {
    ...
    const HolderListMetadata = IDL.Record({ total: IDL.Nat64 });
    const HolderData = IDL.Record({
        account: Account,
        amount: IDL.Nat,
        percentage: IDL.Float64,
    });
    const HolderListResp = IDL.Record({
        metadata: HolderListMetadata,
        data: IDL.Vec(HolderData),
    });
    ...
    return IDL.Service({
        ...
        get_top: IDL.Func([IDL.Nat32], [HolderListResp], ["query"]),
        get_top_100_holder: IDL.Func([], [HolderListResp], ["query"]),
        get_total_holder: IDL.Func([], [IDL.Nat64], ["query"]),
