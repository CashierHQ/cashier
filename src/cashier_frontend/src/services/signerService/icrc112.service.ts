...
export class ICRC112Service {
  ...
  public async icrc112Execute(
    ...
    const arg = {
      ...
      params: {
        ...
        validation: undefined as
          | { canisterId: string; method: string }
          | undefined,
      },
      validation: undefined as
        | { canisterId: string; method: string }
    ...
    const finalResponse: Icrc112Response = {
      responses: arg.params.requests.map((row) =>
    ...
    outerLoop: for (rowIndex = 0; rowIndex < maxRow; rowIndex++) {
      ...
      console.log(
        "🚀 ~ ICRC112Service ~ icrc112Execute ~ rowResponse:",
        rowResponse,
      );
      ...
      for (
        let requestIndex = 0;
        requestIndex < rowRequest.length;
        requestIndex++
      ) {
        ...
        console.log(
          "🚀 ~ ICRC112Service ~ icrc112Execute ~ finalResponse:",
          finalResponse,
        );
      }
    ...
    if (rowHadError) {
      for (let newIndex = rowIndex + 1; newIndex < maxRow; newIndex++) {
        ...
        for (
          let reqIndex = 0;
          reqIndex < nonExecuteParallelRequestRow.length;
          reqIndex++
        ) {
          this.setResponse({
            finalResponse,
