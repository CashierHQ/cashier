...
ctx.addEventListener(
  "message",
  async (event: MessageEvent<FetchMetadataMessage>) => {
      ...
      try {
        ...
        for (let i = 0; i < tokens.length; i += batchSize) {
          ...
          const batchPromises = batch.map(async (token) => {
            ...
            return {
              fee:
                metadata.fee !== undefined ? Number(metadata.fee) : undefined,
              logo: metadata.icon,
        ...
        try {
          ...
          try {
            ctx.postMessage({
              ...
              payload: {
                message:
                  "Failed to send complete results - payload may be too large",
                name: "MessageError",
              },
            });
          } catch (innerError) {
