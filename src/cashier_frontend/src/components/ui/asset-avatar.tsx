...
import {
  ICP_TOKEN_ADDRESS,
  TEST_ICP_TOKEN_ADDRESS,
} from "@/services/fee.constants";
...
export const AssetAvatarV2 = React.forwardRef<
  React.ElementRef<typeof Avatar>,
  AssetAvatarV2Props
>(({ className, ...props }, ref) => {
  const { token } = props;
  ...
  if (
    token.address === ICP_TOKEN_ADDRESS ||
    token.address === TEST_ICP_TOKEN_ADDRESS
  ) {
    logoSrc = ICP_LOGO;
