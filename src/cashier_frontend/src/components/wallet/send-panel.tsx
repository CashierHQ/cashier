...
const SendPanel: React.FC<SendPanelProps> = ({ tokenId, onBack }) => {
  ...
  const [addressType, setAddressType] = useState<"principal" | "account">(
    "principal",
  );
  ...
  const { setSendAssetInfo, openConfirmation, resetSendAsset } =
    useSendAssetStore();
  ...
  const handleToggleUsd = (value: boolean) => {
    ...
    if (value && selectedToken?.usdConversionRate && currentAmount) {
      ...
      const usdValue =
        parseFloat(currentAmount) * selectedToken.usdConversionRate;
      setUsdAmount(usdValue.toString());
    } else if (!value && usdAmount && selectedToken?.usdConversionRate) {
      // Convert USD amount to token
      const tokenValue =
        parseFloat(usdAmount) / selectedToken.usdConversionRate;
      setTokenAmount(tokenValue.toString());
    }
  };
  ...
  return (
    <div className="w-full flex flex-col h-full">
                  ...
                  <FormItem>
                    ...
                    {isIcpToken && (
                      <div className="flex gap-2 mb-2">
                        <button
                          type="button"
                          onClick={() => handleAddressTypeChange("principal")}
                        ...
                        <button
                          type="button"
                          onClick={() => handleAddressTypeChange("account")}
                          className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${
                            addressType === "account"
                              ? "border-2 border-[#36A18B] text-[#36A18B] bg-white"
                              : "border border-gray-200 text-gray-600 bg-white"
      ...
      <SendAssetConfirmationDrawer
        onSuccessfulTransaction={handleTransactionSuccess}
      />
