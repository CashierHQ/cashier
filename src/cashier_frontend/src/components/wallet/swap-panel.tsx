...
const SwapPanel: React.FC<SwapPanelProps> = ({ tokenId, onBack }) => {
    ...
    const [selectedFromToken, setSelectedFromToken] = useState<FungibleToken | undefined>(
        undefined,
    );
    const [selectedToToken, setSelectedToToken] = useState<FungibleToken | undefined>(undefined);
    ...
    const handleFromAmountChange = (value: string) => {
        if (fromIsUsd) {
            ...
            if (selectedFromToken?.usdConversionRate) {
                const tokenValue = parseFloat(value) / selectedFromToken.usdConversionRate;
            ...
            if (selectedFromToken?.usdConversionRate) {
                const usdValue = parseFloat(value) * selectedFromToken.usdConversionRate;
    ...
    const handleToAmountChange = (value: string) => {
        if (toIsUsd) {
            ...
            if (selectedToToken?.usdConversionRate) {
                const tokenValue = parseFloat(value) / selectedToToken.usdConversionRate;
    ...
    return (
        <div className="w-full flex flex-col h-full">
            ...
            <div className="relative flex justify-center items-center mb-4">
                ...
                <h1 className="text-lg font-semibold">{t("wallet.swap.header", "Swap")}</h1>
            </div>
