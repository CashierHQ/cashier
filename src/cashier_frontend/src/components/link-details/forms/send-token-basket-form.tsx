...
export const SendTokenBasketForm = ({
  ...
  const [selectedAssetAddresses, setSelectedAssetAddresses] = useState<
    string[]
  >([]);
  ...
  const handleAssetSelect = createAssetSelectHandler(
    ...
    try {
      if (!link?.id) {
        toast.error(t("common.error"), {
          description: t("error.link.link_id_missing"),
        });
        return;
      }
      ...
      if (!formAssets || formAssets.length === 0) {
        toast.error(t("common.error"), {
          description: t("error.asset.no_assets_found"),
        });
        return;
      }
  ...
  useEffect(() => {
    ...
    ...assetFields.fields.map((field, index) =>
      watch(`assets.${index}.amount`),
    ), // Track all amount changes
    ...assetFields.fields.map((field, index) =>
      watch(`assets.${index}.tokenAddress`),
    ), // Track all tokenAddress changes
  ]);
  ...
  function initializeFirstAsset() {
    ...
    if (link.asset_info && link.asset_info.length > 0) {
      ...
      if (
        label == LINK_INTENT_ASSET_LABEL.INTENT_LABEL_SEND_TOKEN_BASKET_ASSET
      ) {
        label =
          LINK_INTENT_ASSET_LABEL.INTENT_LABEL_SEND_TIP_ASSET +
          "_" +
          firstAsset.address;
      }
  ...
  function getNextAvailableToken(): FungibleToken | undefined {
    if (!displayTokens || displayTokens.length === 0) return undefined;
      ...
      <div>
        ...
        >
          ...
          <button
            ...
            disabled={selectedAssetAddresses.length >= displayTokens.length}
          >
