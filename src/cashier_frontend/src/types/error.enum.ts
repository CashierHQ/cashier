...
const ERROR_CODE_CONFIG: Record<ErrorCode, ErrorCodeConfig> = {
    ...
    [ErrorCode.TIMEOUT_ERROR]: { severity: ErrorSeverity.ERROR, category: ErrorCategory.NETWORK },
    ...
    // Validation errors
    [ErrorCode.REQUIRED]: { severity: ErrorSeverity.WARNING, category: ErrorCategory.VALIDATION },
...
export class CashierError extends Error {
    ...
    constructor(code: ErrorCode, metadata?: ErrorMetadata, message?: string) {
        ...
        if (
            typeof (Error as unknown as { captureStackTrace?: unknown }).captureStackTrace ===
            "function"
        ) {
            (
                Error as unknown as {
