...
export class CashierWalletSignerAdapter extends BaseSignerAdapter<CashierWalletAdapterConfig> {
  ...
  async connect(): Promise<Account> {
    ...
    console.log(
      `[cashier-sdk-adapter] Opening II login popup → ${walletOrigin}`,
    );
    const principal = await this.openLoginPopup(walletOrigin, derivationOrigin);
    this.principalText = principal;
    console.log(
      `[cashier-sdk-adapter] Login complete — principal: ${principal}`,
    );

    // Phase 2 — mount iframe + ICRC-29 transport
    console.log(
      "[cashier-sdk-adapter] Mounting wallet iframe and establishing ICRC-29 channel...",
    );
    ...
    console.log(
      "[cashier-sdk-adapter] Requesting permissions (icrc27_accounts, icrc49_call_canister)...",
    );
    ...
    this.agent = new Proxy(signerAgentRef, {
      get(target: unknown, prop: string | symbol) {
        ...
        if (prop === "call") {
          ...
          return (...args: unknown[]) => {
            console.log(
              "[cashier-sdk-adapter] Update call → routing via ICRC-49 to wallet iframe",
            );
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  protected createActorInternal<T>(
    ...
    if (!this.agent) {
      throw new Error(
        "CashierWalletSignerAdapter: not connected — call connect() first",
      );
    }
