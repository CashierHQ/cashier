...
use gate_service_types::x_response::{OAuthTokenResponse, XProfileResponse, XTweetsResponse};
...
pub(super) fn parse_tweet_id(tweet_url: &str) -> Result<String, GateServiceError> {
    tweet_url
        .rsplit('/')
        .next()
        .filter(|s| !s.is_empty())
...
pub(super) fn decode_tweets_response(
...
// ── Private decoders (exchange_x_token only) ──────────────────────────────────
